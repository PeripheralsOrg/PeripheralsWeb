        ...
        <section class="wrapper">
            ...
            <section class="produto-container">
                ...
                @if (!empty($produtos))
                    ...
                    @foreach ($produtos as $item => $keyProduto)
                    
                        <div style="cursor: pointer;" class="produto-item"
                            onclick="window.location.href=`{{ route('produto-get', $keyProduto['id_produtos']) }}`">

                            <img src="{{ $keyProduto['link_img'] }}" alt="">
                            <hr>
                            <div class="des">
                                <span>{{ $keyProduto['marca'] }}</span>
                                <h5>{{ $keyProduto['nome'] }}</h5>
                                <div class="star">
                                    @for ($i = 0; $i < App\Http\Controllers\ClientProdutoController::getAvaliacaoCarrossel($keyProduto['id_produtos']); $i++)
                                        <i class="fa fa-star"></i>
                                    @endfor
                                </div>
                                <h4>R$ {{ $keyProduto['preco'] }}</h4>
                            </div>
                            <div class="actions-container">
                                <a href="{{ route('carrinho-insert', $keyProduto['id_produtos']) }}"><i
                                        class="fa fa-shopping-bag"></i></a>

                                <a href="{{ route('favoritar-produto', $keyProduto['id_produtos']) }}"><i
